...
export const NotificationsReminderModal = (props: Props) => {
  ...
  const clear = async () => {
    setIsVisible(false);
    await SecureStore.setItem(StorageKeys.NOTIFICATION_PUSH_REMINDED, 'true');
  };
    ...
    >
      ...
      >
        <Text textAlign='center' variant='poppins_l_bold' marginBottom='xs'>
          {t('notifications_reminder')}
        </Text>

        <Icon name='notification' size={ICON_SIZE.xxl} marginBottom='xs' />

        <Text textAlign='center' variant='poppins_m' marginBottom='2xs'>
          {t('forgot_turn_notifications_on')}
        </Text>
